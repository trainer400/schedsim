...
class Scheduler:
    ...
    def get_all_arrivals(self):
        '''
            "Arrivals" is the list of events generated by the added tasks. In case of periodic task, N events are generated
            depending for it, where N depends on the overall simulation time.\n
    ...
    def add_arrivals(self, time_start, time_end):
        '''
            Adds the periodic tasks into the arrival_events list. It is used to extend the simulation time
            (thus periodic tasks must have more events).\n
            @param time_start Starting time to add a new event
            @param time_end Ending time for the time extension
        '''
        for task in self.tasks:
            # Here you can add the code to choose between different cores:
            task.core = self.cores[0].id
    ...
    def find_arrival_event(self, time):
        '''
            ...
            in the start_events list and removed from the arrival_events list. The start_events list is 
            a list of events that are eligible to be executed.\n
    ...
    def find_deadline_events(self, time):
        '''
            Deadline events is the list of real time scheduled events. The method looks for those who
            passed their deadline and notifies it into the output file.\n
            @param time current time
        '''
        helper_list = []
...
class Preemptive(Scheduler):
    ...
    def find_finish_events(self, time):
        '''
            Finish events are generated by events that completed their execution.\n
...
class FIFO(NonPreemptive):
    ...
    def add_time(self, add_time):
        ...
        if pos == -1:
            raise Exception(
                "No saved snapshot to restore, maybe the scheduler has not been run yet")
