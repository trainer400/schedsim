...
def create_graph(output_image, start_time, end_time, fraction):
    try:
        ...
        if os.path.isfile(input_csv) and os.path.getsize(input_csv) > 0:
            ...
            for index, row in df.iterrows():
                ...
                if event_type == 'F':
                    ...
                    if (task_id, job) in active_tasks and 'start' in active_tasks[(task_id, job)]:
                        ...
                        ax.broken_barh([(start_time, duration)], (task_positions[task_id] - 0.2, 0.4), facecolors='tab:green', alpha=0.75)
                    ...
                    ax.broken_barh([(time, 0.1)], (task_positions[task_id] - 0.2, 0.4), facecolors='tab:orange')
            ...
            ax.set_xticks(range(min_time, max_time + 1, fraction))
            ...
            plt.xlabel('Time')  
            plt.ylabel('Task ID')  
            plt.xticks(range(0, end_time, fraction))  
            plt.yticks([])  
